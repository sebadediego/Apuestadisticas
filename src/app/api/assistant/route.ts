...
export async function POST(request: NextRequest) {
  try {
    ...
    // Try Claude first (best), then OpenAI, then Groq
    let response = '';
    if (ANTHROPIC_KEY) {
...
async function callAnthropic(messages: any[]): Promise<string> {
  ...
  try {
    ...
    if (data.error) { console.error('[Anthropic Error]', JSON.stringify(data.error)); return ''; }
    const text = data.content?.map((c: any) => c.text || '').join('') || '';
    if (text) console.log('[Assistant] Anthropic responded OK');
    return text;
  } catch (e: any) {
